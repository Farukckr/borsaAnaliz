...
- Verification: Release build passed with 0 warnings/errors. A real-KAP integration check returned 54 buybacks, 44 dividends, and 30 capital events; every row matched only its declared event set. The three calls shared exactly five upstream requests and repeated calls added none, confirming the cache path. Official KAP disclosure pages 1634548 and 1634534 (dividend) plus 1634576 and 1634471 (capital) all returned HTTP 200. Razor compilation covers all empty states and tab branches; the 375 px behavior is enforced by `flex-nowrap`, horizontal overflow, touch scrolling, and non-wrapping links. A full local HTTP browser run could not start because the configured Supabase connection stalled during startup migration; production HTTP/browser checks remain part of the post-push verification.
